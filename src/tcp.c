...
duk_ret_t dukext_new_tcp(duk_context *ctx) {
  ...
  dukext_check(ctx, uv_tcp_init(dukext_loop(ctx), handle));
...
duk_ret_t dukext_tcp_open(duk_context *ctx) {
  ...
  dschema_check(ctx, (const dukext_schema_entry[]){{"tcp", dukext_is_tcp},
                                                   {"socket", duk_is_number},
                                                   {NULL}});

  handle = duk_get_buffer(ctx, 0, NULL);
  sock = duk_get_number(ctx, 1);
  dukext_check(ctx, uv_tcp_open(handle, sock));
  return 0;
}
...
duk_ret_t dukext_tcp_nodelay(duk_context *ctx) {
  ...
  dschema_check(
      ctx, (const dukext_schema_entry[]){
               {"tcp", dukext_is_tcp}, {"isenabled", duk_is_boolean}, {NULL}});

  handle = duk_get_buffer(ctx, 0, NULL);
  enable = duk_get_boolean(ctx, 1);
  dukext_check(ctx, uv_tcp_nodelay(handle, enable));
  return 0;
}
...
duk_ret_t dukext_tcp_keepalive(duk_context *ctx) {
  ...
  dschema_check(ctx,
                (const dukext_schema_entry[]){{"tcp", dukext_is_tcp},
  ...
  dukext_check(ctx, uv_tcp_keepalive(handle, enable, delay));
  return 0;
}
...
duk_ret_t dukext_tcp_simultaneous_accepts(duk_context *ctx) {
  ...
  handle = duk_get_buffer(ctx, 0, NULL);
  enable = duk_get_boolean(ctx, 1);
  dukext_check(ctx, uv_tcp_simultaneous_accepts(handle, enable));
  return 0;
}
...
duk_ret_t dukext_tcp_bind(duk_context *ctx) {
  ...
  dukext_check(ctx, uv_tcp_bind(handle, (struct sockaddr *)&addr, flags));
  return 0;
}

static void dukext_push_sockaddr(duk_context *ctx,
  ...
  duk_push_object(ctx);
  duk_push_string(ctx, dukext_protocol_to_string(address->ss_family));
...
duk_ret_t dukext_tcp_getsockname(duk_context *ctx) {
  ...
  dschema_check(ctx,
                (const dukext_schema_entry[]){{"tcp", dukext_is_tcp}, {NULL}});

  handle = duk_get_buffer(ctx, 0, NULL);
  addrlen = sizeof(address);
  dukext_check(
      ctx, uv_tcp_getsockname(handle, (struct sockaddr *)&address, &addrlen));
  dukext_push_sockaddr(ctx, &address, addrlen);
  return 1;
}
...
duk_ret_t dukext_tcp_getpeername(duk_context *ctx) {
  ...
  addrlen = sizeof(address);
  dukext_check(
      ctx, uv_tcp_getpeername(handle, (struct sockaddr *)&address, &addrlen));
  dukext_push_sockaddr(ctx, &address, addrlen);
  return 1;
}
...
duk_ret_t dukext_tcp_connect(duk_context *ctx) {
  ...
  dukext_check(ctx, uv_tcp_connect(req, handle, (struct sockaddr *)&addr,
                                   dukext_connect_cb));
  req->data = dukext_setup_req(ctx, 3);
  return 1;
}
