...
duk_bool_t dukext_is_fs(duk_context *ctx, duk_idx_t index) {
  ...
  return size == sizeof(*req) && req->type == UV_FS;
}
...
duk_bool_t dukext_is_stream(duk_context *ctx, duk_idx_t index) {
  ...
  return handle->type == UV_TCP || handle->type == UV_TTY ||
...
duk_bool_t dukext_is_tcp(duk_context *ctx, duk_idx_t index) {
  ...
  handle = duk_get_buffer(ctx, index, &size);
  // TODO: find safer check than size to verify struct type
  return size == sizeof(*handle) && handle->type == UV_TCP;
}
...
duk_bool_t dukext_is_pipe(duk_context *ctx, duk_idx_t index) {
  ...
  handle = duk_get_buffer(ctx, index, &size);
  // TODO: find safer check than size to verify struct type
  return size == sizeof(*handle) && handle->type == UV_NAMED_PIPE;
}
...
duk_bool_t dukext_is_tty(duk_context *ctx, duk_idx_t index) {
  ...
  handle = duk_get_buffer(ctx, index, &size);
  // TODO: find safer check than size to verify struct type
  return size == sizeof(*handle) && handle->type == UV_TTY;
}
