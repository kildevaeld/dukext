...
duk_ret_t dukext_new_pipe(duk_context *ctx) {
  ...
  dschema_check(ctx,
                (const dukext_schema_entry[]){{"ipc", duk_is_boolean}, {NULL}});

  handle = duk_push_fixed_buffer(ctx, sizeof(*handle));
  ipc = duk_get_boolean(ctx, 0);
  dukext_check(ctx, uv_pipe_init(dukext_loop(ctx), handle, ipc));
...
duk_ret_t dukext_pipe_open(duk_context *ctx) {
  ...
  dschema_check(ctx, (const dukext_schema_entry[]){{"pipe", dukext_is_pipe},
                                                   {"fd", dukext_is_fd},
                                                   {NULL}});

  handle = duk_get_buffer(ctx, 0, NULL);
  file = duk_get_int(ctx, 1);
  dukext_check(ctx, uv_pipe_open(handle, file));
  return 0;
}
...
duk_ret_t dukext_pipe_bind(duk_context *ctx) {
  ...
  dukext_check(ctx, uv_pipe_bind(handle, name));
  return 0;
}
...
duk_ret_t dukext_pipe_connect(duk_context *ctx) {
  ...
  dschema_check(ctx,
                (const dukext_schema_entry[]){{"pipe", dukext_is_pipe},
                                              {"name", duk_is_string},
  ...
  uv_pipe_connect(req, handle, name, dukext_connect_cb);
...
duk_ret_t dukext_pipe_getsockname(duk_context *ctx) {
  ...
  dschema_check(
      ctx, (const dukext_schema_entry[]){{"pipe", dukext_is_pipe}, {NULL}});

  handle = duk_get_buffer(ctx, 0, NULL);
  dukext_check(ctx, uv_pipe_getsockname(handle, buf, &len));
